...
class DatabaseValidator:
    ...
    def compare_table(
        ...
        duplicated_fields = set(key_fields) & set(compare_fields)
        if duplicated_fields:
            raise ValueError(
                "Fields cannot be both key and compare fields: "
...
if __name__ == "__main__":
    ...
    # script location /OrderSync/src/database_validator_commande.py  target location /OrderSync
    ...
    logger.info("Checking COMMANDE tables matching")

    check_period_weeks = int(settings.application.maj_periode)

    with (HyperFileConnection(settings) as hyperfile_connection, 
          SqlServerConnection(settings) as sqlserver_connection,
          ):
    ...
    print(result)
    logger.info("Finished checking COMMANDE tables matching.  Result: %s", result)
